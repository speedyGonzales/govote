...
type BordaVoteTest struct {
	Ballots map[string][]byte
}

func newBordaVoteTest() (*BordaVoteTest, *BordaVote) {
	pt := new(BordaVoteTest)
	// candidates := "P,R,S,T"
	...
	return pt, NewBordaVote()
}

//helper function
func (pt *BordaVoteTest) AddBallot(p *BordaVote, bID ...string) error {
	for _, b := range bID {
